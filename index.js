...
async function startPowerplant(page, address, threads) {
    await page.goto("https://powerplant.banano.cc/?r=12625")
...
async function startBananominer(page, address, threads) {
    await page.goto("https://bananominer.arikado.ru/?r=16146")
    await page.waitFor(2000)
    ...
    let currThreadEle = await page.$("#threads")
    let t = parseInt(await page.evaluate(element => element.textContent, currThreadEle))
    ...
    if(!isNaN(t)) {
        ...
        if (t > threads) {
            for(let i = 0; i < t - threads; i++) {
                let btn = await page.$("a#threads-remove")
                await btn.click()
                await page.waitFor(1000)
            }
        }else if (t < threads) {
            for(let i = 0; i < threads - t; i++) {
                let btn = await page.$("a#threads-add")
                await btn.click()
                await page.waitFor(1000)
            }
        }
    ...
    await page.waitFor(2000)
    let btn = await page.$("#start-button a")
    await btn.click()
}
start()